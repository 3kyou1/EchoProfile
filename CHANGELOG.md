...
### Fixed
- Preserve unconfirmed CoPA LLM settings drafts when closing and reopening the settings panel.
- Save CoPA LLM configuration correctly in the Tauri desktop app.
- Keep entertainment Profile history compact by requiring a short title and hiding duplicate long previews.
- Align release validation tests with the current app identifier, updater artifact setting, and Candidate Pools naming.

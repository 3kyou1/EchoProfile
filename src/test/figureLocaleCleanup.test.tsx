...
describe("figure locale cleanup", () => {
  it("uses the explicit wikipedia URL from figure data instead of synthesizing one", () => {
    ...
    const card: FigureResonanceCardType = {
      ...
      wikipedia_url: "https://example.com/custom-ada",
      slug: "ada_lovelace",
    ...
    expect(screen.getByRole("link", { name: "Wikipedia" })).toHaveAttribute(
      "href",
      "https://example.com/custom-ada"
    );
  });

  it("does not render a wikipedia link when figure data has no URL", () => {
    mockUseTranslation.mockReturnValue({
      t: (_key: string, fallback?: string) => fallback ?? "Wikipedia",
      i18n: {
        resolvedLanguage: "en",
